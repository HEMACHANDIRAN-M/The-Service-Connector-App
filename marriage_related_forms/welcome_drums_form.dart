...
class WelcomeDrumsForm extends StatefulWidget {
  const WelcomeDrumsForm({super.key});

  @override
  State<WelcomeDrumsForm> createState() => _WelcomeDrumsFormState();
}

class _WelcomeDrumsFormState extends State<WelcomeDrumsForm> {
  List<String> selectedDrumTypes = [];
  ...
  List<String> PerfomenceDuration = [
    "10-30 Mins",
    "30-60 Mins",
    "1.30 Hours",
    "2 Hours",
  ...
  List<String> Drumtype = [
    "Traditonal Tamil Drums",
    "Kerala Chenda Melam",
    "North Indian Dhol",
    "Fusion Beats",
    "Local Tharathapata ",
  ...
  TextEditingController perfomenceDuration = TextEditingController();
  TextEditingController advancebookingDays = TextEditingController();
  TextEditingController description = TextEditingController();
  TextEditingController drumtype = TextEditingController();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          InkWell(
            onTap: () async {
              final List<String>? results = await showDialog(
                context: context,
                builder: (BuildContext context) {
                  List<String> tempSelected = List.from(selectedDrumTypes);
                  return AlertDialog(
                    backgroundColor: Colors.black,
                    title: Text(
                      "Select Funtion Types",
              ...
              if (results != null) {
                setState(() {
                  selectedDrumTypes = results;
                  drumtype.text = selectedDrumTypes.join(", ");
                });
              }
            },
            child: IgnorePointer(
              child: TextField(
                controller: drumtype,
                decoration: InputDecoration(
                  labelText: "Drum Type (multi-select)",
                  border: OutlineInputBorder(),
                  prefix: Icon(Icons.star, color: Colors.red),
                  suffixIcon: Icon(Icons.arrow_drop_down),
                ),
          ...
          SizedBox(height: 10),
          TextField(
            controller: perfomenceDuration,
            decoration: InputDecoration(
              labelText: "How Long you Perform ?",
              border: OutlineInputBorder(),
              prefix: Icon(Icons.star, color: Colors.red),
              suffixIcon: PopupMenuButton<String>(
                onSelected: (String value) {
                  perfomenceDuration.text = value;
                },
                itemBuilder: (BuildContext context) {
                  return PerfomenceDuration.map((String perfomenceDuration) {
                    return PopupMenuItem<String>(
                      value: perfomenceDuration,
                      child: Text(perfomenceDuration),
                    );
                  }).toList();
                },
              ),
