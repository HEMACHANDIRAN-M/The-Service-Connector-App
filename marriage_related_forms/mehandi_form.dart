...
class MehandiForm extends StatefulWidget {
  const MehandiForm({super.key});

  @override
  State<MehandiForm> createState() => _MehandiFormState();
}

class _MehandiFormState extends State<MehandiForm> {
  List<String> selectedMehandiType = [];
  ...
  void submitForm() async {
    Map<String, dynamic> data = {
      ...
      "mehandiType": mehandiType.text,
      "mehandiStyleLevel": mehandiStyleLevel.text,
      "provideHomeServices": provideHomeServices.text,
  ...
  TextEditingController provideHomeServices = TextEditingController();
  TextEditingController mehandiStyleLevel = TextEditingController();
  TextEditingController mehandiType = TextEditingController();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
            ...
            onTap: () async {
              final List<String>? results = await showDialog(
                ...
                builder: (BuildContext context) {
                  List<String> tempSelected = List.from(selectedMehandiType);
                  return AlertDialog(
                    backgroundColor: Colors.black,
                    title: Text(
                      "Select Mehandi Types",
                      style: TextStyle(color: Colors.white),
                    ),
              ...
              if (results != null) {
                setState(() {
                  selectedMehandiType = results;
                  mehandiType.text = selectedMehandiType.join(", ");
                });
              }
            },
            child: IgnorePointer(
              child: TextField(
                controller: mehandiType,
                decoration: InputDecoration(
                  labelText: "Mehandi Type (multi-select)",
                  border: OutlineInputBorder(),
                  prefix: Icon(Icons.star, color: Colors.red),
          ...
          TextField(
            controller: provideHomeServices,
            decoration: InputDecoration(
              labelText: "Do you Provide HomeServices ? ",
              border: OutlineInputBorder(),
              prefix: Icon(Icons.star, color: Colors.red),
              suffixIcon: PopupMenuButton<String>(
                onSelected: (String value) {
                  provideHomeServices.text = value;
                },
                itemBuilder: (BuildContext context) {
                  return ProvideHomeServices.map((String provideHomeServices) {
                    return PopupMenuItem<String>(
                      value: provideHomeServices,
                      child: Text(provideHomeServices),
                    );
                  }).toList();
                },
              ),
          ...
          SizedBox(height: 10),
          TextField(
            controller: mehandiStyleLevel,
            decoration: InputDecoration(
              labelText: "Mehandi Style Level",
              border: OutlineInputBorder(),
              prefix: Icon(Icons.star, color: Colors.red),
              suffixIcon: PopupMenuButton<String>(
                onSelected: (String value) {
                  mehandiStyleLevel.text = value;
                },
                itemBuilder: (BuildContext context) {
                  return MehandiStyleLevel.map((String mehandiStyleLevel) {
                    return PopupMenuItem<String>(
                      value: mehandiStyleLevel,
                      child: Text(mehandiStyleLevel),
                    );
                  }).toList();
                },
              ),
